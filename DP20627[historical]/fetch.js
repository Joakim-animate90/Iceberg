...
//const links = [];
      ...
      const match = canonicalURL.match(/\?from=(\d{4}-\d{2}-\d{2})\&to=(\d{4}-\d{2}-\d{2})&listaOrgaoJulgador=(.*)/i);
      const match_page = canonicalURL.match(/\?from=(\d{4}-\d{2}-\d{2})\&to=(\d{4}-\d{2}-\d{2})\&listaOrgaoJulgador=(.*)&pg=([0-9]+)/i);
      if (match) {
      ...
      let listaOrgaoJulgador = null
        if (match) {
            listaOrgaoJulgador = match[3]
        }
      ...
      let requestURL;
      if (page) {
        requestURL = `https://www5.tjmg.jus.br/jurisprudencia/pesquisaPalavrasEspelhoAcordao.do?numeroRegistro=1&totalLinhas=1&palavras=&pesquisarPor=ementa&orderByData=2&codigoOrgaoJulgador=&listaOrgaoJulgador=${listaOrgaoJulgador}&codigoCompostoRelator=&classe=&codigoAssunto=&dataPublicacaoInicial=&dataPublicacaoFinal=&dataJulgamentoInicial=${from}&dataJulgamentoFinal=${to}&siglaLegislativa=&referenciaLegislativa=Clique+na+lupa+para+pesquisar+as+refer%EAncias+cadastradas...&numeroRefLegislativa=&anoRefLegislativa=&legislacao=&norma=&descNorma=&complemento_1=&listaPesquisa=&descricaoTextosLegais=&observacoes=&linhasPorPagina=50&pesquisaPalavras=Pesquisar&paginaNumero=${page}`;
