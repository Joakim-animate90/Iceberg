...
const createPdfLinks = async function ({ html, canonicalURL, headers }) {
  ...
  for (let i = 1; i < 9; i++) {
      ...
      let url = `https://www.csj.gov.py/ResolucionesWeb/Formularios/documentoPDF.aspx?resolucionArchivo&nroResolucion=${nroResolucion}&fechaResolucion=${fechaDelaResolucion}`
...
const searchByDate = async function ({ argument, canonicalURL, headers }) {
  ...
  let customHeaders = {
    ...
    "DNT": "1",
    "Origin": "https://www.csj.gov.py",
    "Pragma": "no-cache", "Referer": "https://www.csj.gov.py/ResolucionesWeb/Formularios/inicio.aspx",
    "Sec-Fetch-Dest": "document",
    "Sec-Fetch-Mode": "navigate",
    "Sec-Fetch-Site": "same-origin",
    "Sec-Fetch-User": "?1",
  ...
  if (pageNumber > 1) {
    ...
    if (responsePage.response.status === 200) {
      ...
      return await searchByDate({ argument, canonicalURL, headers })
    }
