...
const applyResultsPerPage = async function ({ canonicalURL, responsePage, headers }) {
    ...
    let _headers = Object.assign(customHeaders, headers);
    const data = {};
    data["hitsperpage-select"] = `argus-HitsPerPage100`;
    let body = querystring.stringify(data);
    let method = "POST";
    let requestOptions = { method, body, headers: _headers };
...
const parseResults = async function ({ responsePage }) {
    ...
    let date = $('.date').text() ? $('.date').text().trim() : null
    ...
    let url = `https://www.bundesanzeiger.de/pub/en/search?name=${name}&area=${area}&infoText=${info}&date=${date}`;
...
const getPages = async function ({ canonicalURL, headers,}) {
    let customHeaders = {
        "Cache-Control": "no-cache",
        "DNT": "1",
        "Pragma": "no-cache",
    ...
    if (checkIfCaptchaNeeded) {
        ...
        responsePage = await postCaptcha({ captchaResult, canonicalURL });


    }
  //return responsePage
    let results = await parseResults({ responsePage });
    let url = results[0].toString();
    let $ = results[1];
...
const pagination = async function ({ arguments, canonicalURL, headers, page }) {
    ...
    let responsePage = await fetchPage({ arguments, canonicalURL, headers });
    //get the location
    await getLocation({ responsePage });
    let location = getSharedVariable('location');
    let requestURL = location;
    responsePage = await fetchPage({ canonicalURL, requestURL, headers });
    //responsePage = await applyResultsPerPage({responsePage, canonicalURL, headers });
    ...
    return [responsePage]
    let results = await parseResults({ responsePage, canonicalURL, headers });
...
const injectHtmlLinks = async function ({ arguments, canonicalURL, headers }) {
    ...
    contentContainer.append(accounting)
...
async function fetchURL({ canonicalURL, headers }) {
    ...
    const isPaginationLinks = canonicalURL.match(/pagination~link/i)
    const isPageLinks = canonicalURL.match(/publication~link/i)
    ...
    } else if (isPaginationLinks) {
        return await pagination({ arguments, canonicalURL, headers })
    } else if (isPageLinks) {
        return [await getPages({ arguments, canonicalURL, headers })]
    } else {
