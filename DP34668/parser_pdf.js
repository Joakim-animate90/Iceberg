...
const removeName = (text, name, out) => {
    ...
    let create_accents = createAccents(words)
    create_accents = words + " " + create_accents

    const regex = new RegExp(`${words}`, 'gui');
    //throw JSON.stringify(words)
...
const createAccents = (text) => {
    ...
    accents.forEach((accent, index) => {
        const baseChar = accent.normalize("NFD").replace(/[\u0300-\u036f]/g, "");
        accentsMap[accent] = baseChar;
    });
    ...
    return text.replace(/[\w']+|./g, (match) => {
        if (match.length === 1) {
            ...
            return match.replace(accentsRegex, (char) => accentsMap[char] || char);
        }
    });
};
...
async function parsePage({ URL, responseBody, html }) {
    ...
    try {
        ...
        if (!/pdf/i.test(responseBody.fileFormat)) {
            ...
            try {
                let out = await parsePageTifDocuments({ URL })
                return out
            } catch (e) {
                return []
            }
        ...
        name = name.replace(/"/g, '')
        name = name.replace(/,/g, '')
        let convertedName = convertToSpanishAccents(name)
...
function convertToSpanishAccents(name) {
    const accentsMap = {
        a: 'á',
        e: 'é',
        i: 'í',
    ...
    for (let i = 0; i < name.length; i++) {
        ...
        convertedName += accentsMap[char] || char;
    }
...
async function parsePageTifDocuments({ URL, }) {
    //return !/pdf/i.test(responseBody.fileFormat)?[{format: responseBody.fileFormat, URL}]:[];//TODO remove this line.
  ...
  `;
    ...
    let pdfMediaObjectId;

    try {
        pdfMediaObjectId = await runRemoteFilterReturnsMediaObjectId({ URL, filter: "tiff2pdf" });

    } catch (err) {

        console.log(err);
    }
    let resp = await graphql(mutationQuery.replace(/MEDIA_OBJECT_ID_GOES_HERE/, pdfMediaObjectId));

    let text = null;
    if (resp && resp.transcodeMediaObject && resp.transcodeMediaObject.mediaObject && resp.transcodeMediaObject.mediaObject.id) {
        text = resp.transcodeMediaObject.mediaObject.content
...
const runRemoteFilterReturnsMediaObjectId = async function({ URL, id, filter }) {
	...
	return node
        ...
        && node.lastSuccessfulRequest.outputForFilter[0].filterOutput.transcodedMediaObject.id;
};
