    ...
    if (/html/i.test(contentType)) {
        ...
        if(/page=1$/i.test(canonicalURL)){
            ...
            for(i = 2; i <= numPages; i++){
                let href = `https://justice.public.lu/fr/jurisprudence/cour-constitutionnelle.html?page=${b}`
                links.push(href)
                b += 20
            }
        ...
        if (content) {
            ...
            let included = json.hits.hits[0]._source.included
            //return [included]
            for(i = 1; i <= included.length; i++){
               // let href = included[i].attributes.isExemplifiedBy
               try{
                 let href = included[i].attributes.isExemplifiedBy
                 href = href['rdfs:Resource']
                 href = href ? url.resolve(requestURL, href) : null;
                if (href) {
                    links.push(href);
                }
               }catch(e){
                console.log('No Access in Attributes')
               }
            }
    }
} 
