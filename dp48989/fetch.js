...
const getHome = async function ({argument, canonicalURL, headers}) {
    ...
    let requestURL = `https://justice.public.lu/fr/jurisprudence/juridictions-administratives.html?q=&eventDate=${from}&eventEndDate=${to}`;
...
const getPagination = async function ({argument, canonicalURL, headers}) {
    ...
    let customHeaders = {
                "Cache-Control": "no-cache",
                "Pragma": "no-cache",
                "Referer": `https://justice.public.lu/fr/jurisprudence/juridictions-administratives.html?q=&eventDate=${from}&eventEndDate=${to}`,
                "Sec-Fetch-Dest": "document",
                "Sec-Fetch-Mode": "navigate",
                "Sec-Fetch-Site": "same-origin",
                "Sec-Fetch-User": "?1",
    ...
    let method = "GET";
    let requestOptions = {method, headers: _headers};
    let requestURL = `https://justice.public.lu/fr/jurisprudence/juridictions-administratives.html?eventEndDate=${to}&eventDate=${from}&b=${page}`;
    let responsePage = await fetchPage({canonicalURL, requestURL, requestOptions});
    return responsePage;
};
