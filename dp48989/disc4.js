function discoverLinks({content, contentType, canonicalURL, requestURL}) {
    ...
    if (/html/i.test(contentType)) {
        ...
        //if page is the same as 1 https://justice.public.lu/fr/jurisprudence/jurisprudence-judoc.html?q=&eventDate=${start}&eventEndDate=${end}&page=1
        if(/page=1$/i.test(canonicalURL)){
            ...
            for(i = 2; i <= numPages; i++){
                let href = `https://justice.public.lu/fr/jurisprudence/jurisprudence-judoc.html?q=&eventDate=${from}&eventEndDate=${to}&page=${b}`
                links.push(href)
                b += 20
            }
