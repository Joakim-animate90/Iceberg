...
async function parsePage({ responseBody, URL }) {
   ...
   // ond div id panel1 get the htmlContent
    ...
    //let $$ = cheerio.load(html);
    //let text = $$('div[id="panel1"]').text().trim();



	 //let output1 = await runRemoteFilter({ id: "A06ruarox30l7zq", filter: "pandocText"}, text);
     //doc.text = output1 && output1.trim() && {content: output1, locale,fileFormat: "text/plain", dataType} || null;
