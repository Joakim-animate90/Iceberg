...
async function parsePage({ responseBody, URL }) {
	...
	for (let i = 0; i < items.length; i++) {
		...
		if (aTextArray.length > 1) {
			doc.docketNumber = aTextArray[0];
			doc.originalNumber = aTextArray[aTextArray.length - 1];	

		}else if (aTextArray.length == 1) {
			doc.docketNumber = aTextArray[0];
		}else {
			doc.docketNumber = null;
			doc.originalNumber = null;
		}
	}
	
	    // let docketNumber be the first element of the array
		// let originalNumber be the last element of the array
    ...
    let newHtml = null
	if (matchIndex !== -1) {
		newHtml = panelHtml.slice(matchIndex);
        panel1 = panel1.html(newHtml)
